...
// Add event listner for F8 key to toggle rotation
...
export default function TouchZoom(eventBus, canvas) {
  ...
  function handleMove(event) {
    ...
    !shouldRotate && canvas.zoom(zoomFactor * context.startZoom, center);

    // Also rotate the canvas according to the change in angle between two fingers
    if (canvas.rotate && shouldRotate) {
      const lastAngle = context.lastAngle || 0;
      var startAngle = Math.atan2(delta[0].y, delta[0].x);
      var endAngle = Math.atan2(delta[1].y, delta[1].x);
      var angle = endAngle - startAngle;

      let adjustedAngle = angle - lastAngle;

      // Adjust to degrees
      adjustedAngle *= 180 / Math.PI;

      canvas.rotate(adjustedAngle, center);

      context.lastAngle = angle;
    }
  }
